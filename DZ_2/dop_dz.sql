...
SELECT DISTINCT
    title, full_text, clients.id, clients.login
FROM
    posts,
    clients,
    users
...
-- Последний запрос не работает, если применён предпоследний запрос. Чтобы проверить №3, надо закомментировать №2.

SELECT users.login, title, full_text 
FROM users, posts 
WHERE users.id = posts.user_id AND users.login = 'Mikle';
